...
class GraphComp:
    ...
    def Create_Bipartite_Giant_Component__x(self, grp, file_name='data/movies.txt', n_movies=40000, prs_out='dictionary', file_type='txt'):
    ...
    def Create_Bipartite_Giant_Component(self, grp, file_name='data/movies.txt', num_limit=500000, n_movies=50000, n_movies_v=20500, prs_out='dictionary', file_type='txt', date_year=2009):
        # grp: Graph_Amazon_Movies.Graph_Amazon()
        start_time = datetime.datetime.now()
        
        max_connected_gr_amazon_movies, max_connected_gr_amazon_movies_val = grp.Create_Bipartite(file_name, num_limit, n_movies, n_movies_v, prs_out, file_type, date_year)
        #Create_Bipartite(file_name, n_movies, n_movies_v, prs_out, self.file_type)
    ...
    def similarity_check_vector_VAL(self, R, ref_user_idx, user, model_info, validation_info, top_similarity=40):
        # !!!!
        # here, we will pass a vector of the interested user from the R matrix, we will not pass the entire matrix
        # it will be like:
        # R[user_index]  and R_zero[user_index]
        ...
        #top_similarity = n_reviews/n_movies
        #top_similarity = 40
        max_connected_gr_amazon_movies = model_info[0]
        movie_indexes = model_info[1]
        user_indexes = model_info[2]
        node_list = model_info[3]
        movies_user_reviewed_already = model_info[4]  # index values
        movie_FOR_reference_user_unique = model_info[5]
        
        max_connected_gr_amazon_movies_v = validation_info[0]
        node_list_v = validation_info[1]    
        top_node_list_v = validation_info[2]
        
        R_array = np.array(list(R))
        if self.debug_mode == 'On':
            print('R matrix values for the user (r vector of the user): \n{}'.format(R_array))

        # sort the values of each column from smaller to the bigger
        R_sorted = np.argsort(R_array)
        
        self.R_sorted = R_sorted

        user_similarity = []
        movie_similarity_rate = []
        movie_similarity_index = []
        movie_similarity_name = []
        
        list_R = R.tolist()[0]

        # we will not consider the user himself or movies as similar nodes
        for idx in R_sorted[0][0]:
            if idx != ref_user_idx and idx not in movie_indexes:
                user_similarity.append(idx)
            if idx not in user_indexes and idx not in movies_user_reviewed_already:
                movie_similarity_index.append(idx)
                movie_similarity_name.append(node_list[idx])
                movie_similarity_rate.append(list_R[idx])
                
        # this need to be normalized
        #newList = [x / myInt for x in myList]
        sum_list = sum(movie_similarity_rate)
        #print('sum_list_movie_similarity_rate: {}'.format(sum_list))
        movie_similarity_rate_norm = [m_rate/sum_list for m_rate in movie_similarity_rate]
        sum_list_norm = sum(movie_similarity_rate_norm)
        #print('sum_list_norm: {}'.format(sum_list_norm))
        
        dict_movie_similarity = {'movie_idx': movie_similarity_index, 'movie_name': movie_similarity_name, 'probability': movie_similarity_rate_norm}
        
        df_movie_similarity = pd.DataFrame(dict_movie_similarity)
                
        user_similarity_top = user_similarity[-top_similarity:]
        top_movie_similarity = movie_similarity_index[-top_similarity:]
        ...
        '''
        ref_movie_list = top_node_list.copy()
        for mv in movies_user_reviewed_already:
            ref_movie_list.remove(mv'
        '''
        
        #user_similarity_top_non_zero = [ref_user_idx]
        for u_idx in user_similarity_top_non_zero:
            user_name = node_list[u_idx]
                          
            #for m in top_node_list_v:
            #for m in ref_movie_list:
            for m in movie_FOR_reference_user_unique:
                #m_idx = node_list.index(m)
        ...
        print(user_review_counts)
        '''
        # movie_similarity_name=movies not seen by the user yet in the dataset1
        return df_summary, movie_FOR_reference_user_unique, movie_similarity_name, user_similarity_top_non_zero, df_movie_similarity
    
    def similarity_check_vector_VAL_x(self, R, movie_indexes, ref_user_idx, node_list, node_list_v, movie_FOR_reference_user, max_connected_gr_amazon_movies, max_connected_gr_amazon_movies_v, top_similarity=40):
        # !!!!
        # here, we will pass a vector of the interested user from the R matrix, we will not pass the entire matrix
        # it will be like:
        # R[user_index]  and R_zero[user_index]
