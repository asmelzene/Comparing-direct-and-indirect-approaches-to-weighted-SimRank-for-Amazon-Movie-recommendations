...
class Amazon:
    ...
    def create_graphs(self, file_name, n_movies, n_movies_v, start_index_v, num_limit=500000, date_year=2009):
        # >>>>>>>>>>>  Preprocessing  <<<<<<<<<<<<<<<
        # >>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<
        ...
        max_connected_gr_amazon_movies, max_connected_gr_amazon_movies_VAL = grpComp.Create_Bipartite_Giant_Component(grp, file_name, num_limit, n_movies, n_movies_v, prs_out, self.file_type, date_year)
        ...
        grpComp.health_check(grp, max_connected_gr_amazon_movies_VAL, bottom_nodes_v, top_nodes_v)
    ...
    def run_validation_TOP_N(self, max_connected_gr_amazon_movies, max_connected_gr_amazon_movies_VAL, walk_steps, n_test_users, beta, top_neighbor, top_N_suggestions):
        ...
        print('FOR loop starts for TEST users (ref users): {}'.format(datetime.datetime.now()))
        movie_FOR_reference_user = list(top_nodes_v)
        ...
        for i, user in enumerate(test_users):
            ...
            #movie_FOR_reference_user_unique = movie_FOR_reference_user.copy()
            movie_FOR_reference_user_unique = top_node_list.copy()
            movies_user_reviewed_already = []
            
            # we shouldn't check a movie which has already been reviewed by the user
            #for mv in lst_common_movies:
            for mv in top_node_list:
                try:
                    if max_connected_gr_amazon_movies.has_edge(user, mv):
                        movie_FOR_reference_user_unique.remove(mv)
                        movies_user_reviewed_already.append(node_list.index(mv))
                except:
                    if self.debug_mode == 'On':
                        print('Movie {} has not been reviewed by user {} in the first (modeling) dataset.'.format(mv, user))
            ...
            model_info = [max_connected_gr_amazon_movies, movie_indexes, user_indexes, node_list, movies_user_reviewed_already, movie_FOR_reference_user_unique]
            
            validation_info = [max_connected_gr_amazon_movies_VAL, node_list_v, top_node_list_v]
            
            # ------ SIMILARITY CHECK
            # movie_by_reference_user_v = movies not seen by the user yet in the dataset1
            df_summary_v, movie_list_ref_v, movie_by_reference_user_v, user_similarity_top_non_zero, df_movie_similarity = \
            grpComp_v.similarity_check_vector_VAL(R_vector, ref_user_idx, user, model_info, validation_info, top_similarity=top_neighbor)
            
            self.df_movie_similarity = df_movie_similarity
            self.R_sorted = grpComp_v.R_sorted
            ...
            '''
            df_summary_ratio_v = \
            grpComp_v.similarity_summary_ratio_VAL(df_summary_v, top_node_list_v, \
                                     max_connected_gr_amazon_movies, max_connected_gr_amazon_movies_VAL'
            '''
            df_summary_ratio_v = \
            grpComp_v.similarity_summary_ratio_VAL(df_summary_v, movie_FOR_reference_user_unique, \
                                     max_connected_gr_amazon_movies, max_connected_gr_amazon_movies_VAL)
            self.df_summary_ratio_v = df_summary_ratio_v
            ...
            '''
            top_K_df_movie_similarity = df_movie_similarity.tail(top_N_suggestions)
            ...
            for mv_new in lst_top_movie_suggestions:
                ...
                ratio_m = 0
                try:
                    #print(df_movie_similarity.query("movie_name=='" + mv_new + "'").values[0].tolist())
                    #print('my_new={}: {}'.format(my_new, df_summary_ratio_v.query("movie=='" + mv_new + "'").values[0].tolist()[1]))
                    #ratio_m = df_movie_similarity.query("movie_name=='" + mv_new + "'").values[0].tolist()[2]
                    ratio_m = df_summary_ratio_v.query("movie=='" + mv_new + "'").values[0].tolist()[1]
                    print('mv_new={}: {}'.format(mv_new, ratio_m))
                    #print('For mv_new:{}: {}'.format(mv_new, df_movie_similarity.query("movie_name=='" + mv_new + "'").values[0].tolist()))
                    lst_ratios.append(ratio_m) 
                except:
                    lst_ratios.append(ratio_m)
                    print('No ratio for the movie: {}'.format(mv_new))
            ...
            # we will iterate in the new movies - ALL    #movie_FOR_reference_user_unique
            for mv_new in top_node_list_v:
                ...
                ratio_m_all = 0
                try:
                    ratio_m_all = df_movie_similarity.query("movie_name=='" + mv_new + "'").values[0].tolist()[2]
                    lst_ratios_all.append(ratio_m_all) 
                except:
                    lst_ratios_all.append(ratio_m_all)
    ...
    def run_validation_TOP_N_x(self, max_connected_gr_amazon_movies, max_connected_gr_amazon_movies_VAL, walk_steps, n_test_users, beta, top_neighbor, top_N_suggestions):
        start_time = datetime.datetime.now()
    ...
    def save_performance_values_ALL(self, df_FINAL, time_1, time_2):
        ...
        print('fpr, tpr, thresholds  starts: {}'.format(datetime.datetime.now()))
        fpr, tpr, thresholds = metrics.roc_curve(lst_true_val, lst_ratio_val)  #, pos_label=2
        lst_tpr = list(tpr); lst_fpr = list(fpr); lst_thresholds_base = list(thresholds)
        
        '''
        lst_thresholds = []
        if len(lst_thresholds_base) > 15:
            lst_thresholds = list(np.random.uniform(0, 1, 15))
        else:
            lst_thresholds = lst_thresholds_base
        '''
        ...
        #acc, tpr, fpr, tnr, ppv, f1 = final_summary(df_FINAL_copy) # 7 comes from here, 6 + threshold
        max_lst_length = max(min(len(lst_tpr), 15), len(lst_name), 6)   

        print('roc_auc starts: {}'.format(datetime.datetime.now()))
        roc_auc = roc_auc_score(lst_true_val, lst_ratio_val)
        #precision = pre
        ...
        print('lst_thresholds starts: {}'.format(datetime.datetime.now()))
        
        '''
        for t in lst_thresholds:
            acc, tpr, fpr, tnr, fnr, ppv, f1 = self.final_summary_threshold_based(df_FINAL, t)
            lst_t = [t,  acc, tpr, fpr, tnr, fnr, ppv, f']
            lst_presicion_based.append(lst_t)
        '''
        ...
        print('lst_presicion_based starts: {}'.format(datetime.datetime.now()))
        '''
        lst_precision = list(np.zeros(max_lst_length, str))
        for i, l in enumerate(lst_presicion_based):
            p_precision = round(l[-2], 6)
        ...
        #lst_other_metrics_value_prep = [threshold,  acc, tpr, fpr, tnr, fnr, ppv, f1]
        #lst_other_metrics_value_prep = lst_presicion_based[max_i]
        lst_other_metrics_value_prep = [0]
        lst_value[7] = lst_other_metrics_value_prep[0]
        
        print('lst_other_metrics_name_prep starts: {}'.format(datetime.datetime.now()))
        for i, val in enumerate(lst_other_metrics_name_prep):
            lst_other_metrics_name[i] = val
        for i, val in enumerate(lst_other_metrics_value_prep):
            lst_other_metrics_value[i] = val
        ...
        dict_perf = {'AUC': lst_auc, 'Calc_Time': lst_calc_time, 'lst_name': lst_name, 'lst_value': lst_value,
                    'other_metrics': lst_other_metrics_name, 'metrics_val': lst_other_metrics_value}
